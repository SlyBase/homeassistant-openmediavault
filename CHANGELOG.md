...
- Raising ruff's `target-version` to `py314` (to match the new Python floor) made its formatter adopt PEP 758 unparenthesized multi-exception syntax; reformatted the two affected `except (TypeError, ValueError):` clauses in `binary_sensor_types.py` and `coordinator.py` to `except TypeError, ValueError:` to pass `ruff format --check` in CI.
- The pre-commit hook (`.pre-commit-config.yaml`) pinned `ruff-pre-commit` to `v0.9.10`, which predates the `py314` target-version value and rejected the config outright; bumped it to `v0.16.5` to match the version already pinned in `pyproject.toml`'s dev dependencies.
