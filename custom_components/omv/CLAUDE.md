...
Applies to `coordinator.py` in this directory.
...
3. **Register** the result under a new key in `unfiltered_data`. Document the key in the root `CLAUDE.md`.
...
- Each normalised record must carry a stable `*_key` field (e.g. `disk_key`, `container_key`, `project_key`) that doubles as the unique identifier used by `filter_data_by_selection` and `build_inventory`.
